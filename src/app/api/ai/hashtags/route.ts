...
export async function POST(request: NextRequest) {
  ...
  const platformInfo = platforms?.length
    ...
    : "";
  const message = `${platformInfo}Generate hashtags for the following post content:\n\n${content}`;
    ...
    async start(controller) {
      ...
      try {
        ...
        try {
          ...
          result = JSON.parse(jsonMatch[0]) as HashtagResult;
        } catch {
          result = {
            hashtags: {
              primary: [],
            ...
            platformSpecific: {},
            reasoning: "Hashtag generation failed. Please try again.",
          };
        }
